...
public class ControlaInterface : MonoBehaviour {
	...
	public Text TextoQuantidadeDeZumbiMortos;
		...
		public void AtualizarQuantidadeDeZumbisMortos ()
	{
		...
		TextoQuantidadeDeZumbiMortos.text = string.Format("x {0}" , quantidadeDeZumbisMortos);
	...
	public void GameOver()
	{
		...
		TextoTempoDeSobrevivencia.text = 
			"Voce sobreviveu por " + minutos + "min e " + segundos + "s";
	...
	void AjustarPontuacaoMaxima(int min, int seg)
	{
		if(Time.timeSinceLevelLoad > tempoPontuacaoSalvo) 
		{
			...
			TextoPontuacaoMaxima.text = 
				string.Format("Seu melhor tempo e {0}min e {1}s", min, seg);
	...
	{
		SceneManager.LoadScene("Zombie curso game");
	}

	public void AparecrTextoChefeCriado ()
	{
